#!/usr/bin/env bash
# Jobverse: fix listProspects to actually return which ATS a prospect is on.
#
# apiListProspects_ never returned an `ats` field - only a Status flag
# ('Found' vs 'Found - Manual Review') existed to say whether Prospects.gs
# recognised the ATS, but never *which* one, by name, to a caller. worker.js
# (batch 3) depends on that field to pick the right ats/*.js module - without
# this fix every prospect.ats was undefined, so the worker could never match
# a Greenhouse posting to worker/ats/greenhouse.js even when it should.
#
# Adds:
#   - appscript/Api.js: detectATSName_ (same domains Prospects.gs's own
#     detectATSFromUrl_ checks, returns the name instead of a boolean) and
#     wires it into apiListProspects_'s output.
#   - worker/worker.js: checks ATS support BEFORE calling analyseJob, so a
#     Lever/Ashby/NHS prospect (recognised by Prospects.gs, but with no
#     worker/ats/*.js module yet) doesn't burn a real AI-generated CV/cover
#     letter and an Applications row before the worker discovers it can't
#     act on it. Left Queued either way, so it's picked up automatically
#     whenever that module ships - no need to re-queue by hand.
#
# Usage: run from inside your jobverse-repo clone:
#   bash apply-batch-4.sh
...
echo "Rewriting appscript/Api.js (apiListProspects_ now returns ats)..."
...
echo "Rewriting worker/worker.js (checks ATS support before analyseJob)..."
...
echo "Committing..."
git add -A
git commit -m "Fix apiListProspects_ to return ats, so the worker can match modules

apiListProspects_ never returned which ATS a prospect was on - only a
Status flag existed ('Found' vs 'Found - Manual Review'). worker.js reads
prospect.ats to pick between ats/greenhouse.js and ats/workday.js; without
this it was always undefined, so nothing would ever match. Also makes the
worker check ATS support before calling analyseJob, so unsupported
prospects (Lever/Ashby/NHS - recognised by Prospects.gs, but no worker
module yet) don't waste a real AI generation call.
...
echo ""
echo "Done. Now: clasp push to deploy Api.js, then restart the worker (Ctrl+C,"
echo "npm start again) if it was already running so it picks up the fix."
